...
appH.AppView = Backbone.View.extend({
	...
	initialize: function() {
		...
		this.listenTo(appH.Todos, 'add', this.addOne);
		this.listenTo(appH.Todos, 'reset', this.addAll);

		this.listenTo(appH.Todos, 'change:completed', this.filterOne);
		this.listenTo(appH.Todos, 'filter', this.filterAll);

		// all event to bind any event triggered on the Todos collection to the view’s render method
		this.listenTo(appH.Todos, 'all', this.render);
	...
	render: function() {
		...
		if (appH.Todos.length) {
			...
			this.$('#filters li a')
				...
				.filter('[href="#/' + (appH.TodoFilter || '') + '"]') // The value of appH.TodoFilter will be set by our router
	...
	addAll: function() {
		...
		this.$('#todo-list').html('');
		appH.Todos.each(this.addOne, this);
	},
	...
	filterAll: function() {
		...
		appH.Todos.each(this.filterOne, this);
	},
	...
	createOnEnter: function(event) {
		...
		appH.Todos.create(this.newAttributes());
	...
	clearCompleted: function() {
		...
		_.invoke(appH.Todos.completed(), 'destroy');
		return false;
	},
